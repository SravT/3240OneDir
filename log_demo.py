...
''' Logging notes
The logging code is injected into three sections (connectionMade, connectionLost, lineReceived)
import json, and import collections is needed
The logging data is saved in admin_log.txt in JSON format. If you ever need to remake a new admin_log.txt,
make sure to initialize the file by typing in {"logCount": 0} in the admin_log.txt file
'''
...
class FileTransferProtocol(basic.LineReceiver):
	...
	def connectionMade(self):
		...
		logMessage = 'Connection from: %s (%d clients total)' % (self.transport.getPeer().host, len(self.factory.clients))
		self.log(logMessage)
	...
	def connectionLost(self, reason):
		...
		logMessage = 'Connection from %s lost (%d clients left)' % (self.transport.getPeer().host, len(self.factory.clients))
		self.log(logMessage)
	...
	def log(self, logMessage):
		# First retrieve the current log
		stored_data = "admin_log.txt"
		with open(stored_data, "r+") as f:
			output = json.load(f)
		f.closed

		# Prepare new items to add to log
		logCount = output['logCount']
		lineDictionary = {logCount: logMessage}
		logCount += 1
		logCountDictionary = {"logCount": logCount}

		# Sort the log
		tempArray = []
		outputDictionary = {}
		for i in xrange(0, len(output)-1):
			tempArray.append(output.get(str(i)))
		for i in xrange(0, len(tempArray)):
			outputDictionary[i] = tempArray[i]
		
		# Finally, add the new items to the log and update the file
		outputDictionary = dict(outputDictionary.items() + lineDictionary.items() + logCountDictionary.items())
		with open(stored_data, "r+") as f:
			input = json.dump(outputDictionary, f)
		f.closed
